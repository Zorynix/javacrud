CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_customers_email_partial ON customers(email) WHERE email IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_customers_type_created ON customers(customer_type, created_at);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_order_items_product_quantity ON order_items(product_id, quantity);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_active_stock ON products(stock_quantity) WHERE status = 'ACTIVE';
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_delivered_revenue ON orders(total_amount, delivered_at) WHERE status = 'DELIVERED';
